...
export async function home_page() {
    ...
    try {
        ...
        if (cart && cart.getTotalQty() !=0) {
            cart.items.forEach(item => {
                const p = product.find(e => e.docId == item.docId);
                if (p) p.qty = item.qty;
            })
        }
    } catch (e) {
    ...
    for (let i = 0; i < product.length; i++) {
        html += buildProductView(product[i], i);
    }
    ...
    for (let i=0; i < productForms.length; i++) {
        productForms[i].addEventListener('submit', e => {
            e.preventDefault();
            const p = product[e.target.index.value];
            ...
            <div class="container pt-3 bg-light ${currentUser ? 'd-block' : 'd-none'}">
                <form method="post" class="form-product-qty">
                    ...
                    <div id="item-count-${product.docId}"
                        class="container rounded text-center text-white bg-success d-inline-block w-50">
